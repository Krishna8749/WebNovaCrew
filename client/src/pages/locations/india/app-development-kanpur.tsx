...
export default function AppDevelopmentKanpur() {
  ...
  return (
    <div className="min-h-screen bg-violet-950 font-sans">
      <SEO 
        title="App Development Company in Kanpur | Mobile App Developers UP | ₹25,000 Onwards | XcFlown"
        description="Top-rated app development company in Kanpur, UP. We build Android, iOS, Flutter apps for leather, textile, e-commerce industries. 40-50% lower than Delhi rates. Free consultation. Call +91 83607 23410."
      ...
      <BreadcrumbSchema items={[
        ...
        { name: "Kanpur", url: "https://xcflown.com/locations/app-development-kanpur" }
      ]} />
      <ServiceSchema 
        ...
        serviceUrl="https://xcflown.com/locations/app-development-kanpur"
      />
        ...
        <div className="container mx-auto max-w-7xl px-4 relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-violet-500/10 border border-violet-500/20 rounded-full mb-6">
              ...
              <span className="text-sm text-violet-300">Kanpur, Uttar Pradesh • Industrial Capital of UP • 3M+ Population</span>
            </div>
            ...
            <p className="text-xl md:text-2xl text-slate-300 mb-4 leading-relaxed">
              Build powerful <span className="text-violet-400 font-semibold">Android, iOS & Flutter apps</span> for your Kanpur business. Specialized in <span className="text-orange-400 font-semibold">leather, textile & manufacturing</span> industry apps.
            </p>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-4xl mx-auto">
              <motion.div 
                ...
                className="text-center bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-violet-500/20"
              >
                <div className="text-3xl md:text-4xl font-bold text-violet-400 mb-1">50+</div>
              ...
              <motion.div 
                ...
                className="text-center bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-violet-500/20"
              >
                <div className="text-3xl md:text-4xl font-bold text-violet-400 mb-1">3-4</div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {services.map((service, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="bg-violet-800/50 border border-violet-700 rounded-xl p-6 hover:border-violet-500/50 transition-all group"
              >
                <div className="w-14 h-14 bg-violet-500/10 rounded-xl flex items-center justify-center text-violet-400 mb-4 group-hover:bg-violet-500/20 transition-all">
                  {service.icon}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {service.features.map((feature, i) => (
                    <span key={i} className="text-xs bg-violet-500/10 text-violet-300 px-2 py-1 rounded">
                      {feature}
                    </span>
                  ))}
                </div>
          ...
          <div className="flex flex-wrap gap-4 justify-center max-w-5xl mx-auto">
            {areas.map((area, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className="px-5 py-3 bg-violet-800/50 border border-violet-700 rounded-full text-slate-300 hover:border-violet-500/50 transition-all flex items-center gap-2"
              >
                <MapPin className="w-4 h-4 text-violet-400" />
                <span>{area.name}</span>
            ...
            {process.map((step, index) => (
              ...
              >
                ...
                {index < process.length - 1 && (
                  <div className="hidden md:block absolute top-1/2 -right-2 w-4 h-0.5 bg-violet-500/50"></div>
                )}
              </motion.div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {pricing.map((plan, index) => (
              <motion.div
                ...
                transition={{ delay: index * 0.1 }}
                viewport={{ once: true }}
                className={`bg-violet-800/50 border rounded-xl p-6 relative ${
                  plan.popular ? 'border-violet-500 ring-2 ring-violet-500/20' : 'border-violet-700'
                }`}
              >
                {plan.popular && (
                ...
                <div className="text-3xl font-bold text-violet-400 mb-6">{plan.price}</div>
          ...
          <div className="grid md:grid-cols-4 gap-6 max-w-4xl mx-auto">
            {techStack.map((stack, index) => (
              <motion.div
                ...
                transition={{ delay: index * 0.1 }}
                viewport={{ once: true }}
                className="bg-violet-800/50 border border-violet-700 rounded-xl p-5"
              >
                <h3 className="text-lg font-semibold text-white mb-3">{stack.category}</h3>
                <div className="flex flex-wrap gap-2">
                  {stack.techs.map((tech, i) => (
                    <span key={i} className="text-sm bg-violet-500/10 text-violet-300 px-3 py-1 rounded-full">
                      {tech}
                    </span>
                  ))}
                </div>
        ...
        <div className="container mx-auto max-w-4xl px-4 text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <a href="tel:+918360723410">
              <Button size="lg" variant="outline" className="border-violet-700/50 hover:bg-violet-800 text-white px-8 py-6 text-lg">
