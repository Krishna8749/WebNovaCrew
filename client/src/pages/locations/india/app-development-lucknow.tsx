...
export default function AppDevelopmentLucknow() {
  ...
  const faqs = [
    ...
    {
      question: "Is XcFlown based in Lucknow?",
  ...
  return (
    <div className="min-h-screen bg-violet-950 font-sans">
      <SEO title="App Development Company in Lucknow | Mobile App Developers UP | XcFlown" description="Top app development company in Lucknow - Our HQ. Android, iOS, Flutter apps for businesses, government in Gomti Nagar, Hazratganj. ₹25,000 onwards." keywords="app development Lucknow, mobile app developers Lucknow, Gomti Nagar app development, UP app developers" />
      <LocalBusinessSchema /><BreadcrumbSchema items={[{ name: "Home", url: "https://xcflown.com" }, { name: "Locations", url: "https://xcflown.com/locations" }, { name: "Lucknow", url: "https://xcflown.com/locations/app-development-lucknow" }]} />
      <ServiceSchema serviceName="App Development in Lucknow" serviceDescription="Professional mobile app development - Our headquarters in Lucknow." serviceUrl="https://xcflown.com/locations/app-development-lucknow" />
      <Navbar />
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {services.map((service, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}
                className="bg-violet-800/50 border border-violet-700 rounded-xl p-8 hover:border-emerald-500/50 transition-all">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {areas.map((area, index) => (
              <motion.div key={index} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ delay: index * 0.03 }}
                className="bg-violet-800/50 border border-violet-700 rounded-xl p-5 hover:border-emerald-500/50 transition-all">
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
            {pricing.map((plan, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}
                className={`bg-violet-800/50 border rounded-xl p-8 transition-all ${plan.popular ? 'border-emerald-500 ring-2 ring-emerald-500/20' : 'border-violet-700 hover:border-emerald-500/50'}`}>
