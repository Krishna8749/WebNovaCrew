...
export default function WebDevelopmentAlambagh() {
  ...
  const faqs = [
    { question: "Which is the best web development company in Alambagh?", answer: "XcFlown is the top web development company in Alambagh, Lucknow. We specialize in hospitality, transport, and retail business websites that capture Alambagh's high transit traffic. Starting at ₹12,000 with 1-week delivery." },
  ...
  return (
    <div className="min-h-screen bg-violet-950 font-sans">
      <SEO title="Web Development Company in Alambagh, Lucknow | Hotels & Transport | XcFlown" description="Top web development company in Alambagh, Lucknow. Hotel booking websites, transport apps, restaurant platforms. ₹12,000 onwards. Capture transit traffic." keywords="web development company in Alambagh, app developer in Alambagh Lucknow, digital marketing agency Alambagh, hotel website Alambagh, transport app Alambagh, website developer near Alambagh" />
      <LocalBusinessSchema />
      <BreadcrumbSchema items={[{ name: "Home", url: "https://xcflown.com" }, { name: "Lucknow Areas", url: "https://xcflown.com/locations/app-development-lucknow" }, { name: "Alambagh", url: "https://xcflown.com/locations/web-development-alambagh" }]} />
      <ServiceSchema serviceName="Web Development in Alambagh" serviceDescription="Professional web and app development services in Alambagh, Lucknow. Hotel booking platforms, transport apps, restaurant websites, and local SEO for transit traffic." serviceUrl="https://xcflown.com/locations/web-development-alambagh" />
      <FAQSchema faqs={faqs} />
      <Navbar />
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-sky-950/30 to-slate-950">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-[200px] h-[200px] md:w-[300px] md:h-[300px] lg:w-[350px] lg:h-[350px] bg-violet-500/20 rounded-full blur-[50px] lg:blur-[80px]"></div>
        </div>
        <div className="container mx-auto max-w-7xl px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 max-w-3xl mx-auto">
              ...
              <div className="text-center bg-white/5 rounded-xl p-4 border border-sky-500/20"><div className="text-3xl font-bold text-violet-400 mb-1">₹12K</div><div className="text-sm text-slate-400">Starting Price</div></div>
              ...
              <div className="text-center bg-white/5 rounded-xl p-4 border border-sky-500/20"><div className="text-3xl font-bold text-violet-400 mb-1">40%</div><div className="text-sm text-slate-400">More Bookings</div></div>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="bg-violet-800/50 border border-violet-700 rounded-xl p-8 hover:border-sky-500/50 transition-all">
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {caseStudies.map((study, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-violet-700 rounded-xl p-8 hover:border-sky-500/50 transition-all">
      ...
      <section className="py-16 bg-violet-900/50 border-t border-violet-800">
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {nearbyAreas.map((area, index) => (<Link key={index} href={`/locations/${area.slug}`}><a className="px-5 py-2.5 bg-violet-800/50 border border-violet-700 rounded-full text-slate-300 hover:border-sky-500/50 hover:text-sky-400 transition-all"><MapPin className="w-4 h-4 inline mr-2" />{area.name}</a></Link>))}
          </div>
        </div>
      </section>
