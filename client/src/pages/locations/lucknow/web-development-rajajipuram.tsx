...
export default function WebDevelopmentRajajipuram() {
  ...
  const faqs = [
    { question: "Which is the best affordable web development company in Rajajipuram?", answer: "XcFlown offers the most affordable web development in Rajajipuram, Lucknow. Basic business websites start at ₹12,000 with free hosting for the first year and WhatsApp integration included." },
  ...
  return (
    <div className="min-h-screen bg-violet-950 font-sans">
      <SEO title="Affordable Web Development in Rajajipuram, Lucknow | XcFlown" description="Budget-friendly web development company in Rajajipuram, Lucknow. Websites from ₹12,000. Grocery apps, shop websites, delivery solutions. Free hosting 1st year. 40+ Rajajipuram clients." keywords="web development company in Rajajipuram, app developer in Rajajipuram Lucknow, digital marketing agency Rajajipuram, affordable website Rajajipuram, grocery app developer Rajajipuram" />
      <LocalBusinessSchema />
      <BreadcrumbSchema items={[{ name: "Home", url: "https://xcflown.com" }, { name: "Lucknow Areas", url: "https://xcflown.com/locations/app-development-lucknow" }, { name: "Rajajipuram", url: "https://xcflown.com/locations/web-development-rajajipuram" }]} />
      <ServiceSchema serviceName="Web Development in Rajajipuram" serviceDescription="Affordable web and app development services in Rajajipuram, Lucknow. Grocery delivery apps, shop websites, food ordering platforms, and budget-friendly digital marketing." serviceUrl="https://xcflown.com/locations/web-development-rajajipuram" />
      <FAQSchema faqs={faqs} />
      <Navbar />
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="bg-violet-800/50 border border-violet-700 rounded-xl p-8 hover:border-fuchsia-500/50 transition-all">
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {caseStudies.map((study, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-violet-700 rounded-xl p-8 hover:border-fuchsia-500/50 transition-all">
      ...
      <section className="py-16 bg-violet-900/50 border-t border-violet-800">
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {nearbyAreas.map((area, index) => (<Link key={index} href={`/locations/${area.slug}`}><a className="px-5 py-2.5 bg-violet-800/50 border border-violet-700 rounded-full text-slate-300 hover:border-fuchsia-500/50 hover:text-fuchsia-400 transition-all"><MapPin className="w-4 h-4 inline mr-2" />{area.name}</a></Link>))}
          </div>
        </div>
      </section>

      <section className="py-20 bg-gradient-to-br from-fuchsia-950/50 to-pink-950/50 border-t border-violet-800">
