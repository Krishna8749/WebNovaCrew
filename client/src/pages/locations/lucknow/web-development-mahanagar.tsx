...
export default function WebDevelopmentMahanagar() {
  ...
  const faqs = [
    { question: "Which is the best web development company in Mahanagar?", answer: "Web Nova Crew is the premium web development company in Mahanagar, Lucknow. We create high-end websites and apps for upscale businesses, clinics, and professional offices. Starting at ₹30,000 with premium design quality." },
  ...
  return (
    <div className="min-h-screen bg-slate-950 font-sans">
      <SEO title="Premium Web Development in Mahanagar, Lucknow | Web Nova Crew" description="Premium web development company in Mahanagar, Lucknow. High-end websites, boutique e-commerce, wellness apps. ₹30,000 onwards. Upscale business specialist." keywords="web development company in Mahanagar, app developer in Mahanagar Lucknow, digital marketing agency Mahanagar, premium website developer Mahanagar, boutique website Mahanagar" />
      <LocalBusinessSchema />
      <BreadcrumbSchema items={[{ name: "Home", url: "https://webnovacrew.com" }, { name: "Lucknow Areas", url: "https://webnovacrew.com/locations/app-development-lucknow" }, { name: "Mahanagar", url: "https://webnovacrew.com/locations/web-development-mahanagar" }]} />
      <ServiceSchema serviceName="Web Development in Mahanagar" serviceDescription="Premium web and app development services in Mahanagar, Lucknow. High-end websites, boutique e-commerce platforms, wellness booking apps, and sophisticated digital marketing." serviceUrl="https://webnovacrew.com/locations/web-development-mahanagar" />
      <FAQSchema faqs={faqs} />
      <Navbar />

      <section className="relative pt-32 pb-20 overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-cyan-950/30 to-slate-950">
          <div className="absolute top-1/4 left-1/4 w-[250px] h-[250px] md:w-[350px] md:h-[350px] lg:w-[450px] lg:h-[450px] bg-cyan-500/20 rounded-full blur-[60px] lg:blur-[100px]"></div>
        ...
        <div className="container mx-auto max-w-7xl px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="text-center max-w-4xl mx-auto">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-cyan-500/10 border border-cyan-500/20 rounded-full mb-6">
              ...
              <span className="text-sm text-cyan-300">Mahanagar, Lucknow • Premium Residential • Upscale Business</span>
            </div>
            ...
            <p className="text-xl md:text-2xl text-slate-300 mb-4 leading-relaxed">Mahanagar's <span className="text-cyan-400 font-semibold">premium businesses</span> deserve sophisticated digital presence. We build <span className="text-teal-400 font-semibold">high-end websites, boutique stores & wellness apps</span>.</p>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 md:gap-6 max-w-3xl mx-auto">
              <div className="text-center bg-white/5 rounded-xl p-4 border border-cyan-500/20"><div className="text-3xl font-bold text-cyan-400 mb-1">45+</div><div className="text-sm text-slate-400">Premium Clients</div></div>
              <div className="text-center bg-white/5 rounded-xl p-4 border border-cyan-500/20"><div className="text-3xl font-bold text-teal-400 mb-1">₹30K</div><div className="text-sm text-slate-400">Starting Price</div></div>
              <div className="text-center bg-white/5 rounded-xl p-4 border border-cyan-500/20"><div className="text-3xl font-bold text-cyan-400 mb-1">2-3</div><div className="text-sm text-slate-400">Weeks Website</div></div>
              <div className="text-center bg-white/5 rounded-xl p-4 border border-cyan-500/20"><div className="text-3xl font-bold text-teal-400 mb-1">250+</div><div className="text-sm text-slate-400">Projects Delivered</div></div>
            </div>
          </motion.div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {services.map((service, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="bg-slate-800/50 border border-slate-700 rounded-xl p-8 hover:border-cyan-500/50 transition-all">
                <div className="w-14 h-14 bg-cyan-500/10 rounded-xl flex items-center justify-center text-cyan-400 mb-4">{service.icon}</div>
                <h3 className="text-xl font-semibold text-white mb-3">{service.title}</h3>
                <p className="text-slate-400 mb-4 leading-relaxed">{service.desc}</p>
                <div className="flex flex-wrap gap-2">{service.features.map((f, i) => (<span key={i} className="text-xs bg-cyan-500/10 text-cyan-300 px-3 py-1 rounded-full">{f}</span>))}</div>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {caseStudies.map((study, index) => (
              <motion.div key={index} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-slate-700 rounded-xl p-8 hover:border-cyan-500/50 transition-all">
                <div className="text-cyan-400 text-sm font-medium mb-2">{study.client}</div>
                <h3 className="text-xl font-bold text-white mb-4">{study.title}</h3>
                <p className="text-slate-400 mb-6 leading-relaxed">{study.description}</p>
                <div className="grid grid-cols-2 gap-3 mb-6">{study.results.map((r, i) => (<div key={i} className="bg-cyan-500/10 rounded-lg px-3 py-2 text-center"><span className="text-cyan-300 text-sm font-medium">{r}</span></div>))}</div>
      ...
      <section className="py-16 bg-slate-900/50 border-t border-slate-800">
        <div className="container mx-auto max-w-7xl px-4">
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            {nearbyAreas.map((area, index) => (<Link key={index} href={`/locations/${area.slug}`}><a className="px-5 py-2.5 bg-slate-800/50 border border-slate-700 rounded-full text-slate-300 hover:border-cyan-500/50 hover:text-cyan-400 transition-all"><MapPin className="w-4 h-4 inline mr-2" />{area.name}</a></Link>))}
          </div>
        </div>
      </section>
