  ...
  return (
    <div className="min-h-screen bg-violet-950 font-sans">
      ...
      <BreadcrumbSchema items={[
        ...
        { name: "Flutter App Development", url: "https://xcflown.com/services/flutter-app-development" }
      ]} />
      <ServiceSchema 
        ...
        serviceUrl="https://xcflown.com/services/flutter-app-development"
      />
        ...
        <div className="container mx-auto max-w-7xl px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-4xl mx-auto">
            ...
            <p className="text-xl text-violet-200/80 mb-8 leading-relaxed">
              ...
              <strong className="text-violet-400"> single codebase</strong>. Native performance, stunning UI, 40% cost savings. 100+ Flutter apps delivered.
            </p>
      ...
      <section className="py-24 bg-violet-950">
        <div className="container mx-auto max-w-7xl px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Why Choose <span className="text-violet-400">Flutter</span>?</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {benefits.map((benefit, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <p className="text-violet-200/60 text-sm">{benefit.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-gradient-to-b from-slate-950 to-blue-950/20">
        <div className="container mx-auto max-w-7xl px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Our Flutter <span className="text-violet-400">Services</span></h2>
          </div>
          <div className="flex flex-wrap justify-center gap-4">
            {services.map((service, i) => (
              <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
                className="bg-white/5 border border-violet-500/20 rounded-full px-6 py-3 text-white font-semibold hover:border-violet-500/50 transition-all">
                {service}
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-24 bg-violet-950">
        <div className="container mx-auto max-w-6xl px-4">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-5xl font-bold text-white mb-6">Flutter App <span className="text-violet-400">Pricing</span></h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {pricing.map((tier, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                className={`relative bg-gradient-to-br from-slate-800/50 to-slate-900/50 border rounded-3xl p-8 ${tier.popular ? 'border-violet-500 scale-105' : 'border-white/10'}`}>
                ...
                <div className="mb-2"><span className="text-2xl font-bold text-violet-400">{tier.price}</span><span className="text-violet-200/60 ml-2">🇮🇳</span></div>
                <div className="mb-6"><span className="text-lg font-bold text-orange-400">{tier.priceKWD}</span><span className="text-violet-200/60 ml-2">🇰🇼</span></div>
                <ul className="space-y-3 mb-8">
                  {tier.features.map((feature, j) => (
                    <li key={j} className="flex items-center gap-3 text-violet-200/80"><CheckCircle2 className="w-5 h-5 text-violet-400 shrink-0" />{feature}</li>
                  ))}
                </ul>
      ...
      <section className="py-24 bg-gradient-to-r from-blue-600 to-cyan-600">
        <div className="container mx-auto max-w-4xl px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Link href="/contact"><Button size="lg" className="bg-white text-violet-600 hover:bg-violet-50 font-bold text-lg px-10 py-7 rounded-full">Start Your Project <ArrowRight className="w-5 h-5 ml-2" /></Button></Link>
