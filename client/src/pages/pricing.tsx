...
export default function Pricing() {
  ...
  return (
    <div className="min-h-screen bg-violet-950 font-sans">
      <SEO 
        title="App Development Pricing India & Kuwait | Transparent Costs | XcFlown" 
      ...
      <BreadcrumbSchema items={[
        ...
        { name: "Pricing", url: "https://xcflown.com/pricing" }
      ]} />
      ...
      <section className="pt-32 pb-16 relative overflow-hidden">
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-blue-950/30 to-slate-950">
          <div className="absolute top-1/4 left-1/4 w-[500px] h-[500px] bg-orange-500/10 rounded-full blur-[100px]"></div>
        </div>
        
        <div className="container mx-auto max-w-6xl px-4 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center">
            <span className="inline-flex items-center gap-2 bg-orange-500/20 text-orange-400 px-5 py-2 rounded-full text-sm font-bold mb-6 border border-orange-500/30">
              💰 Transparent Pricing
            </span>
            ...
            <p className="text-xl text-violet-200/80 max-w-3xl mx-auto mb-8">
            ...
            <div className="inline-flex items-center bg-white/10 rounded-full p-1 mb-12">
              <button
                ...
                className={`px-6 py-3 rounded-full font-bold transition-all ${currency === "INR" ? "bg-orange-500 text-white" : "text-white/70 hover:text-white"}`}
              >
                🇮🇳 INR (India)
              </button>
              <button
      ...
      {appPricing.map((category, catIndex) => (
        <section key={catIndex} className={`py-16 ${catIndex % 2 === 0 ? "bg-violet-950" : "bg-violet-900/50"}`}>
          <div className="container mx-auto max-w-7xl px-4">
            ...
            >
              <div className="w-14 h-14 bg-gradient-to-br from-violet-600 to-orange-500 rounded-2xl flex items-center justify-center text-white">
                {category.icon}
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {category.tiers.map((tier, tierIndex) => (
                ...
                >
                  {tier.popular && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-gradient-to-r from-violet-600 to-orange-500 text-white text-xs font-bold px-4 py-1 rounded-full">
                      Most Popular
                    </div>
                  )}
                  ...
                  <div className="text-sm text-violet-200/60 mb-4">Timeline: {tier.timeline}</div>
                  <ul className="space-y-2 mb-6">
                    {tier.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-2 text-violet-200/80 text-sm">
