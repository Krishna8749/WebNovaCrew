...
export async function registerRoutes(
  ...
  // Terabox API routes — same-origin guard + human browser guard (blocks AdSense/bots)
  app.post("/api/terabox/resolve", teraboxGuard, requireHumanBrowser, (req, res, next) => {
  ...
  app.post("/api/terabox/share", teraboxGuard, requireHumanBrowser, (req, res, next) => {
    void handleTeraboxShareCreate(req, res).catch(next);
  });

  // Public read — bots get 403, humans get share data
  app.get("/api/terabox/share/:id", requireHumanBrowser, (req, res, next) => {
    void handleTeraboxShareGet(req, res).catch(next);
  });

  app.get("/api/terabox/share-config", requireHumanBrowser, (req, res, next) => {
